...
from multitask.dataloader import MultiTaskDataset
...
DECODER_NAMES = sorted(
    name for name in decoder_models.__dict__ if not name.startswith("__") and callable(decoder_models.__dict__[name]))
...
def load_model(model_path, best_weights=False):
    ...
    model_last_ckpt = torch.load(last_ckpt_path, map_location='cpu')
    ...
    if best_weights:
        ...
        # best_model_path = '../model/multitask/resnet50_ResNetBackbone_CTGNN_GCN/Fixed-Effective-MTL-version_1/epoch=20-val_loss=6.22.ckpt'
        best_model_path = './model/multitask/resnet50_ResNetBackbone_CTGNN_GCN/Fixed-Effective-MTL-version_1/epoch=20-val_loss=6.22.ckpt'
...
def MTL_inference(args):
    ...
    updated_encoder_state_dict, updated_decoder_state_dict, model_name, hparams, backbone_hparam, encoder_hparam, decoder_hparam, task_classes, valid_tasks = load_model(
    ...
    if decoder_hparam in DECODER_NAMES:
        if decoder_hparam == "SimpleHead":
            decoder = decoder_models.SimpleHead(n_tasks=len(task_classes), num_classes=task_classes,
                                                input_channels=input_channels,
                                                decoder_channels=hparams["decoder_channels"],
                                                pool=hparams["decoder_pool"], list_input=list_input)
        elif decoder_hparam == "CTGNN":
            decoder = decoder_models.CTGNN(n_tasks=len(task_classes), num_classes=task_classes,
                                           input_channels=input_channels, decoder_channels=hparams["decoder_channels"],
                                           adj_mat_path=hparams["adj_mat_path"], gnn_head=hparams["gnn_head"],
                                           gnn_layers=hparams["gnn_layers"], gnn_channels=hparams["gnn_channels"],
                                           gnn_dropout=hparams["gnn_dropout"], gnn_residual=hparams["gnn_residual"],
                                           attention_heads=hparams["gat_num_heads"],
                                           bottleneck_channels=hparams["bottleneck_channels"],
                                           adj_normalization=hparams["adj_normalization"], pool=hparams["decoder_pool"],
                                           list_input=list_input, shared_bottleneck=hparams["shared_bottleneck"],
                                           shared_linear=hparams["shared_linear"],
                                           gnn_residual_act=hparams["gnn_residual_act"])
    else:
    ...
    if "defect" in valid_tasks:
        ...
        sigmoid_defect_df.to_csv(
            os.path.join(outputPath, "{}_defect_{}_sigmoid.csv".format(model_version, split.lower())), sep=",",
            index=False)
    ...
    if "water" in valid_tasks:
        ...
        sigmoid_water_dict.to_csv(
            os.path.join(outputPath, "{}_water_{}_sigmoid.csv".format(model_version, split.lower())), sep=",",
            index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path", type=str, default='../model/multitask')
