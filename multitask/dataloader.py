...
class MultiTaskDataset(Dataset):
    def __init__(self, annRoot, imgRoot, split="Train", transform=None, loader=default_loader,
                 waterIntervals=[5, 15, 30], onlyDefects=False, inferce=False):
    ...
    def loadAnnotations(self):
        ...
        gt = pd.read_csv(gtPath, sep=",", encoding="utf-8",
                         usecols=self.defect_LabelNames + ["Filename", "Defect", "WaterLevel"])
        ...
        if len(self.waterIntervals) > 0:
            ...
            for idx in range(1, len(self.waterIntervals)):
                self.water_labels[(self.water_labels >= self.waterIntervals[idx - 1]) & (
                            self.water_labels < self.waterIntervals[idx])] = idx
        else:
...
class WaterLevelDataset(Dataset):
    ...
    def loadAnnotations(self):
        ...
        if len(self.waterIntervals) > 0:
            ...
            for idx in range(1, len(self.waterIntervals)):
                self.labels[
                    (self.labels >= self.waterIntervals[idx - 1]) & (self.labels < self.waterIntervals[idx])] = idx
        else:
            uniqueLevels = np.unique(self.labels)
