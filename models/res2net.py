...
class Res2Net(nn.Module):
    ...
    def __init__(self, block, layers, baseWidth=26, scale=4, num_classes=1000):
        ...
        self.lay1_feat = nn.Conv2d(256, 2048, kernel_size=8, padding=1, stride=8)
        self.lay2_feat = nn.Conv2d(512, 2048, kernel_size=4, padding=1, stride=4)
        self.lay3_feat = nn.Conv2d(1024, 2048, kernel_size=3, padding=1, stride=2)
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)
        lay1 = x
        lay1 = self.lay1_feat(lay1)
        lay_feat.append(lay1)
        x = self.layer2(x)
        lay2 = x
        lay2 = self.lay2_feat(lay2)
        lay_feat.append(lay2)
        x = self.layer3(x)
        lay3 = x
        lay3 = self.lay3_feat(lay3)
        lay_feat.append(lay3)
        x = self.layer4(x)
