...
class CTGNN(nn.Module):
    def __init__(self, n_tasks, num_classes, input_channels, decoder_channels, adj_mat_path, gnn_head, gnn_layers=1,
                 gnn_channels=128, gnn_dropout=0.6, gnn_residual=True, attention_heads=None, adj_normalization="",
                 pool="Avg", list_input=False, shared_bottleneck=False, shared_linear=True, gnn_residual_act="Pre",
                 **kwargs):
        ...
        self.construct_adjacency_matrix(adj_normalization, adj_mat_path='./multitask/adj_all_65/adj_binary.npy')
    ...
    def construct_adjacency_matrix(self, adj_normalization, adj_mat_path):
        ...
        if adj_normalization == "In":
            ...
            raise ValueError(
                "The provided adjacency normalization method is not available: {}".format(adj_normalization))
    ...
    def construct_feature_projections(self):
        ...
        for idx in range(len(self.num_classes)):
            self.feat_projs.append(nn.Sequential(
                *[nn.Linear(self.penultimate_channels, self.gnn_channels * self.num_classes[idx], bias=True),
                  nn.LeakyReLU(inplace=True, negative_slope=0.2)]))

    def construct_ct_gnn(self):

        if self.gnn_head == "GCN":
            self.ct_gnn = nn.ModuleList([GCNLayer(in_features=self.gnn_channels, dropout=self.gnn_dropout,
                                                  alpha=self.alpha, residual=self.gnn_residual,
                                                  residual_act=self.gnn_residual_act) for _ in range(self.gnn_layers)])
        elif self.gnn_head == "GAT":
            self.ct_gnn = nn.ModuleList([GATLayer(in_features=self.gnn_channels, attention_heads=self.attention_heads,
                                                  dropout=self.gnn_dropout, alpha=self.alpha,
                                                  residual=self.gnn_residual, residual_act=self.gnn_residual_act,
                                                  concat=True) for _ in range(self.gnn_layers)])
        else:
    ...
    def construct_linear_projection(self):
        if self.shared_linear:
            ...
            self.linear_class = nn.ModuleList(
                [nn.Linear(self.gnn_channels, 1, bias=True) for _ in range(sum(self.num_classes))])
    ...
    def forward(self, x, lays_feat):
        ...
        # node_feats_1 = torch.stack(node_feats, dim=1) # [B, 21, 128]
        # node_feats = lay_feats[0]
        # for layer in range(self.gnn_layers):
        #     if layer > 0 and layer < 3:
        #         node_feats += lay_feats[layer]
        #     if layer == 3:
        #         node_feats += node_feats_1
